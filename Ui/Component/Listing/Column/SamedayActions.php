...
class SamedayActions extends Column
{
    /**
     * @var BulkAwbService
     */
    ...
    public function prepare()
    {
        ...
        $fieldClass = isset($config['fieldClass']) && is_array($config['fieldClass'])
            ? $config['fieldClass']
            : [];
        $fieldClass['col-sameday_actions'] = true;
        $config['fieldClass'] = $fieldClass;
        // Prevent Magento's default row "view order" fieldAction from firing on Select.
        ...
        foreach ($dataSource['data']['items'] as &$item) {
            ...
            try {
                $item[$this->getData('name')] = $orderId
                    ? $this->bulkAwbService->formatActionsHtml($orderId)
                    : '';
            } catch (\Exception $e) {
